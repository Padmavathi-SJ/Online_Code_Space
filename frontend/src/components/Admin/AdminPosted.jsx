...
import '../../App.css';
import '../../css/Admin/AdminPosted.css';
...
const AdminPostedQuestions = ({ userId }) => {
  ...
  return (
    <div className="admin-posted-container">
      <h2 className="admin-posted-title">Posted Questions</h2>
      {loading ? (
        <p>Loading tests...</p>
      ) : error ? (
        <p>{error}</p>
        ...
        <div className="tests-grid">
          {tests.map(test => (
            <div key={test._id} className="test-box" onClick={() => handleTestClick(test._id)}>
              ...
              <button className="delete-btn" onClick={(e) => { e.stopPropagation(); handleDelete(test._id); }}>Delete</button>
            </div>
          ))}
        </div>
      )}
